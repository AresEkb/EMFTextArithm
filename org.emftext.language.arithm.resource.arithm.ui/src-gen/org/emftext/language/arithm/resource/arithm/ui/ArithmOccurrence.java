 ...
 */
public class ArithmOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.arithm.resource.arithm.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.arithm.resource.arithm.ui.occurences.declaration";
	...
	private org.emftext.language.arithm.resource.arithm.ui.ArithmTokenScanner tokenScanner;
	 ...
	 */
	public ArithmOccurrence(org.emftext.language.arithm.resource.arithm.IArithmTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.arithm.resource.arithm.ui.ArithmTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.arithm.resource.arithm.mopp.ArithmMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.arithm.resource.arithm.ui.ArithmBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.language.arithm.resource.arithm.ui.ArithmPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.language.arithm.resource.arithm.ui.ArithmPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.language.arithm.resource.arithm.ui.ArithmPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.language.arithm.resource.arithm.ui.ArithmPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.language.arithm.resource.arithm.ui.ArithmOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.arithm.resource.arithm.ui.ArithmOccurrence.DECLARATION_ANNOTATION_ID);
	}
