...
/**
 * The ArithmTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * ArithmTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class ArithmTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.emftext.language.arithm.resource.arithm.mopp.ArithmTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.emftext.language.arithm.resource.arithm.mopp.ArithmTaskItem> foundItems = new java.util.ArrayList<org.emftext.language.arithm.resource.arithm.mopp.ArithmTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.arithm.resource.arithm.mopp.ArithmTaskItem(keyword, message, localLine, offset, end));
