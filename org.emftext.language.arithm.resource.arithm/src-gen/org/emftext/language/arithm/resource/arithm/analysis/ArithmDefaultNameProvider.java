...
public class ArithmDefaultNameProvider implements org.emftext.language.arithm.resource.arithm.IArithmNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.arithm.resource.arithm.util.ArithmEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
